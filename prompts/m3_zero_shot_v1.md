# M3 zero-shot prompt specification v1

Prompt version: `m3-zero-shot-v1`  
Method: `M3`  
Status: frozen experiment-preparation specification; no API output has been generated.

The request builder reads the marked block below as the shared developer
instruction. The corresponding block in the M4 specification must remain
byte-identical. Prompt development must not use A1 test labels or any A2
...
1. the shared marked block as one developer message; and
2. one user message containing exactly one target Fact Summary in the common
   JSON-encoded target wrapper.

It contains no demonstrations. The strict schema and decoding settings come
from `config/experiments/llm_extraction_v1.yaml`.
